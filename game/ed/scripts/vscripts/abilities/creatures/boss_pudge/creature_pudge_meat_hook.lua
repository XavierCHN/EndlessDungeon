creature_pudge_meat_hook = class({})
LinkLuaModifier( "modifier_pudge_meat_hook_followthrough", "modifiers/creatures/boss_pudge/modifier_pudge_meat_hook_followthrough.lua", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_creature_pudge_meat_hook", "modifiers/creatures/boss_pudge/modifier_creature_pudge_meat_hook.lua", LUA_MODIFIER_MOTION_HORIZONTAL )

--------------------------------------------------------------------------------

function creature_pudge_meat_hook:OnAbilityPhaseStart()
...
function creature_pudge_meat_hook:OnAbilityPhaseInterrupted()
    self:GetCaster():RemoveGesture( ACT_DOTA_OVERRIDE_ABILITY_1 )
end

--------------------------------------------------------------------------------

function creature_pudge_meat_hook:OnSpellStart()
    ...
    self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_pudge_meat_hook_followthrough", { duration = flFollowthroughDuration } )
...
function creature_pudge_meat_hook:OnProjectileHit( hTarget, vLocation )
    ...
    if self.bRetracting == false then
        ...
        if hTarget ~= nil then
            ...
            hTarget:AddNewModifier( self:GetCaster(), self, "modifier_creature_pudge_meat_hook", nil )

            if hTarget:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
                local damage = {
                    victim      = hTarget,
                    attacker    = self:GetCaster(),
                    damage      = hTarget:GetMaxHealth() * 0.2,
        ...
        if self.hVictim ~= nil then
            ...
            self.hVictim:InterruptMotionControllers( true )
            self.hVictim:RemoveModifierByName( "modifier_creature_pudge_meat_hook" )
...
function creature_pudge_meat_hook:OnProjectileThink( vLocation )
    self.vProjectileLocation = vLocation
end

--------------------------------------------------------------------------------

function creature_pudge_meat_hook:OnOwnerDied()
    self:GetCaster():RemoveGesture( ACT_DOTA_OVERRIDE_ABILITY_1 );
