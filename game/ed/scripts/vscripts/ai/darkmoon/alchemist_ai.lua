...
function AIThink()
    -- For some reason AddThinkToEnt doesn't accept member functions
...
function BehaviorConcoction:Evaluate()
    ...
    if self.concoctionAbility and self.concoctionAbility:IsFullyCastable() then
        self.target = AICore:ClosestPlayerHeroInRange( thisEntity, self.concoctionAbility:GetCastRange() )
...
function BehaviorAcidSpray:Evaluate()
    ...
    if self.acidSprayAbility and self.acidSprayAbility:IsFullyCastable() then
        self.target = AIUtil:RandomPlayerHeroInRange( thisEntity, self.acidSprayAbility:GetCastRange() )
        if self.target and self.target:IsAlive() then
            if self.target:IsStunned() then
                desire = 6
            else
